...
// Input: solution URL + (customer URL OR industry) + email (auth only).
// Output: 3 pains, 2 strategies, 2 discovery questions, 3-email drip.
// Target: under 25 seconds even on cold cache.

const SMB_PROMPT = `You are an elite B2B sales strategist. Given a solution and a target customer (or customer archetype), produce a complete small-deal sales brief in one pass.
...
app.post('/api/smb-flow', async (req, res) => {
  ...
  try {
    // ── PHASE 1: Ingest solution, customer separately ──
    send('phase', { phase: 'fetch', message: 'Fetching solution and target…' });

    const solutionPromise = ingestOne({ url: normUrl(solution_url), role: 'solution' });

    // Customer: URL → single-page ingest, industry → archetype, neither → minimal
    const customerPromise = customer_url
      ? ingestOne({ url: normUrl(customer_url), role: 'customer' })
      : industry
        ? synthesizeCustomerArchetype({ industry, region })
        : Promise.resolve({
    ...
    const [solutionRes, customerRes] = await Promise.allSettled([
      solutionPromise, customerPromise
    ]);
    ...
    const allCached = [solution, customer].every(e => ['local_cache', 'db_cache'].includes(e.source));
    ...
    db.saveRun(run_id, {
      email, solution_url, customer_url,
      industry, region
    }, {
      solution, customer,
      smb_result: result
